    ...
    try {
      ...
      const items = Array.isArray(list) ? list.map(item => `
        <div class="w-i">
          ...
          <button class="w-d" onclick="window.download(decodeURIComponent('${enc(item.name ?? "")}'))" title="Download">⬇️</button>
          <button class="w-d" onclick="(async()=>{await window.purge(decodeURIComponent('${enc(item.name ?? "")}'));location.reload()})()" title="Delete">🗑️</button>
        </div>`
      ).join('') : '';
